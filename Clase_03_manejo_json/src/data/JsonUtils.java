...
public class JsonUtils<T> {
    ...
    public void updateByName(T updatedItem, String name) throws IOException {
        Map<String, T> map = getElements((Class<T>) updatedItem.getClass());
        if (map.containsKey(name)) {
            map.put(name, updatedItem); // Actualiza el elemento
            mapper.writeValue(new File(filePath), map);
        } else {
            throw new IllegalArgumentException("No se encontró un elemento con el nombre: " + name);
        }
    }

    // Eliminar un elemento por nombre
    public void deleteByName(String name, Class<T> temp) throws IOException {
        Map<String, T> map = getElements(temp);
        if (map.remove(name) != null) { // Remueve el elemento si existe
            mapper.writeValue(new File(filePath), map);
        } else {
            throw new IllegalArgumentException("No se encontró un elemento con el nombre: " + name);
        }
